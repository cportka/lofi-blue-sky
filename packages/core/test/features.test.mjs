...
const KEYS = ['Palette', 'Split', 'Band Density', 'Movement', 'Drift', 'Processing', 'True Horizon', 'Full Corruption'];
...
test('deriveFeatures is deterministic and complete', () => {
  for (let i = 0; i < 200; i++) {
    ...
    assert.equal(typeof f['True Horizon'], 'boolean');
...
const base = {
  ...
  bands: 20, hbands: 1, movement: 'distorted', blocks: false, blocksN: 8,
...
test('features discriminate at their thresholds', () => {
  ...
  assert.equal(deriveFeatures(mk({ movement: 'classic' })).Movement, 'Classic');
  assert.equal(deriveFeatures(mk({ movement: 'distorted' })).Movement, 'Distorted');
  // Band Density at 24
  assert.equal(deriveFeatures(mk({ bands: 23 }))['Band Density'], 'Wide');
  assert.equal(deriveFeatures(mk({ bands: 24 }))['Band Density'], 'Fine');
  // Drift at pulse-strength 0.06 (clean movements drop the smear term, so they read calmer)
  ...
  assert.equal(deriveFeatures(mk({ movement: 'true-clean', bandDrift: 0.05, rowDisplace: 0.045 })).Drift, 'Still'); // 0.05, smear ignored
  ...
  // True Horizon window (horizon in (0.40, 0.52), rowDisplace < 0.025) — now visualized in sky.ts
  assert.equal(deriveFeatures(mk({ horizon: 0.41, rowDisplace: 0.02 }))['True Horizon'], true);
  assert.equal(deriveFeatures(mk({ horizon: 0.46, rowDisplace: 0.024 }))['True Horizon'], true);
  assert.equal(deriveFeatures(mk({ horizon: 0.46, rowDisplace: 0.03 }))['True Horizon'], false);
  assert.equal(deriveFeatures(mk({ horizon: 0.55, rowDisplace: 0.01 }))['True Horizon'], false);
